...
export default function AdminUsuariosPage({ token }: { token: string }) {
  const router = useRouter();
  const { page = '1', q = '' } = router.query;
  const { mutate } = useSWRConfig();
  const swrKey = token ? [`/api/proxy/admin/users?page=${page}&q=${q}`, token] : null;
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/admin/usuarios?page=${newPage}&q=${searchTerm}`);
  };

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/admin/usuarios?page=1&q=${searchTerm}`);
  };

  const handleDelete = async (userId: string) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este usuario? Sus anuncios quedarán sin propietario.')) {
      try {
        await axios.delete(`/api/proxy/admin/users/${userId}`, {
  ...
  const handleRoleChange = async (userId: string, currentRole: string) => {
    ...
    if (window.confirm(`¿Estás seguro de que quieres cambiar el rol de este usuario a "${newRole}"?`)) {
      try {
        await axios.patch(`/api/proxy/admin/users/${userId}/role`, 
          { role: newRole },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        mutate(swrKey);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Head>
        <title>ComidaRapida - Gestionar Usuarios</title>
      </Head>
      <Header />
      ...
      <main className="flex-grow max-w-6xl mx-auto p-4">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center">
            ...
            <Link href="/admin/usuarios/crear" className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg">
              Crear Nuevo Usuario
            </Link>
          </div>
        </div>
                      ...
                      <td className="px-6 py-4">
                        <button onClick={() => handleRoleChange(user.id, user.role)}>
                          {user.role === 'admin' ? (
                            ...
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Cliente</span>
                          )}
                        </button>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/usuarios/editar/${user.id}`} className="text-indigo-600 hover:text-indigo-900">
                          Editar
                        </Link>
                        <button onClick={() => handleDelete(user.id)} className="ml-4 text-red-600 hover:text-red-900">
                          Eliminar
                        </button>
                      </td>
                    </tr>
