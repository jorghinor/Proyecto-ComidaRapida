...
export default function EditarAnuncioPage({ token }: { token: string }) {
  ...
  const { data: foodTypes, error: foodTypesError } = useSWR('/api/proxy/food-types', (url) => axios.get(url).then(res => res.data));
  const { data: categories, error: categoriesError } = useSWR('/api/proxy/categories', (url) => axios.get(url).then(res => res.data));
  const { data: cities, error: citiesError } = useSWR('/api/proxy/cities', (url) => axios.get(url).then(res => res.data));

  const { data: adData, error: adError } = useSWR(
    id && token ? [`/api/proxy/public/ads/${id}`, token] : null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`/api/proxy/ads/${id}`, formData, config);
  ...
  const dataLoadingError = adError || foodTypesError || categoriesError || citiesError;
  const isDataMissing = !adData || !foodTypes || !categories || !cities;

  if (dataLoadingError) {
    console.error("Error loading form data:", dataLoadingError);
    return (
      <div className="min-h-screen flex flex-col">
        <Header />
        <main className="flex-grow container mx-auto px-4 py-8 text-red-500">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <select id="foodType" value={foodTypeId} onChange={(e) => setFoodTypeId(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                {foodTypes?.map((type: FoodType) => <option key={type.id} value={type.id}>{type.name}</option>)}
              </select>
            </div>
            <div>
              ...
              <select id="category" value={categoryId} onChange={(e) => setCategoryId(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                {categories?.map((cat: Category) => <option key={cat.id} value={cat.id}>{cat.name} (Precio: ${cat.price}, Duración: {cat.durationDays} días)</option>)}
              </select>
            </div>
            <div>
